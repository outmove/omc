...
pub const ADDRESS_LENGTH: usize = omv_primitives::account_address::AccountAddress::LENGTH;

#[derive(Ord, PartialOrd, Eq, PartialEq, Hash, Default, Clone, Copy)]
pub struct Address([u8; ADDRESS_LENGTH]);

impl Address {
    pub const DIEM_CORE: Address = Address::new(omv_primitives::language_storage::CORE_CODE_ADDRESS.to_u8());
    ...
    fn try_from(bytes: &[u8]) -> Result<Address, String> {
        if bytes.len() != ADDRESS_LENGTH {
            ...
            if bytes.len() != addr.len() {
                return Err(format!("can't copy source slice into destination slice: sizes don't match"));
            }
