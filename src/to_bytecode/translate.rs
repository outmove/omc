...
use omv_primitives::account_address::AccountAddress as DiemAddress;
use omc_ir_types::{ast as IR, location::*};
use omv_core::file_format as F;
...
fn module(
    ...
    let (module, source_map) = omc_ir_to_bytecode::compiler::compile_module(addr, ir_module, deps)
...
fn script(
    ...
    let deps: Vec<&F::CompiledModule> = vec![];
    let (script, source_map) = omc_ir_to_bytecode::compiler::compile_script(None, ir_script, deps)
